  ...
  <div class="flex flex-col items-center space-y-6 p-8">
    <div class="w-full max-w-2xl">
      {#each strings as { note, key, color }}
        <button
          class="relative w-full h-16 bg-gradient-to-r from-amber-800 to-amber-700 rounded-lg mb-4 overflow-hidden transition-transform duration-200 transform hover:scale-x-[1.02] focus:outline-none focus:ring-2 focus:ring-amber-500
            {activeStrings.has(key) ? 'scale-x-[1.01]' : ''}"
        ...
        >
          ...
          <div class={`absolute inset-0 bg-${color}/20 transform origin-left transition-transform duration-300 ${activeStrings.has(key) ? 'scale-x-100' : 'scale-x-0'}`}>
          </div>
        </button>
      {/each}
    </div>
